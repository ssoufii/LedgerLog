...
## Story M7.2: Multi-SSTable get(): newest-write-wins

Looking up a key now searches everywhere the store keeps data, not just the part held in memory. It checks the batch currently being written to, then any recent batches waiting to be saved, then every file on disk from the newest to the oldest, and it stops at the first place that has an answer. That order is what makes the answer the right one: the same key can sit in several files at once with an old value in one and a newer value in another, and stopping at the first hit means the most recent write is the one that comes back. A deletion counts as an answer too, so a key deleted after it was saved reads as missing rather than falling back to the older value underneath it.

The search also tries not to do work it does not need to. Each file carries a small summary that can say for certain that a key is not in it, so files that cannot hold the key are skipped without being opened at all, and a key that was never stored costs almost nothing to look for. The tests check each of these claims separately, including counting how many files were actually opened during a lookup, since a search that opened every file would still give the right answer and only be slow. One test arranges for a file to be saved in the middle of a single lookup, and others run several readers against a writer while files are being saved in the background, to confirm that a key is never briefly invisible while it moves from memory to disk.

## Story M8.1: Size-tier grouping and compaction trigger

The store writes a new file to disk every time the batch it holds in memory fills up, so the number of files on disk keeps growing, and a lookup for a key that is not in memory may have to check every one of them. The fix is to merge files together now and then, and this story is the part that decides which files should be merged and when. Files of a similar size are put into the same group, and a group is flagged as worth merging once it holds enough files, four by default. Both of those numbers are settings rather than fixed rules, so they can be tuned later against real measurements.

Grouping by size is what keeps the work affordable. Merging four small files into one costs very little and removes three files a lookup would otherwise have to check, whereas mixing a large file in with small ones would mean rewriting the large file over and over for no gain. The rule for "similar size" is deliberately set so that the result of merging a group is bigger than the group it came from, which is what stops the same data being rewritten forever. The grouping is recalculated from scratch whenever a new file appears, so what it reports is always about the files that exist right now, and the tests check it against groupings worked out by hand, against thousands of randomly generated size combinations, and against a store being written to by several threads at once while files are saved in the background. Nothing is merged yet: this story only produces the recommendation, and the merge itself comes next.
